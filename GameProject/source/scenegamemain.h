...
public:
	int		_cg;		// 画像
	int		_w, _h;		// 画像サイズ
	int		_color_r = 0, _color_g = 0, _color_b = 0, _color_a = 0;		// 色
...
//#define EFFECT_TYPE_SPARK_YELLOW 132
	...
	int pl_x;
	int pl_y;
	int pl_stand;	//プレイヤーが地面についてるかどうか。0：ついてない、1：ついてる
	int pl_arrow;
	int pl_gravity; 
	...
	int _bossbgm;
	int bgmMum = 0;
	int IsHitStop;		//ヒットストップするかどうか
	int CheckHitStop;	//ヒットストップしたかどうか
};
